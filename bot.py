...
def active_owner(d, uid):
    """Чей задачник сейчас активен. 'self' = свой. None = по умолчанию
...
def main_kb():
    return {"keyboard": [
        [{"text": "🏠 Мой чат"}, {"text": "💬 Выбрать чат"}],
        [{"text": "🎙 Напомнить"}, {"text": "📝 Запомнить"}],
        [{"text": "📋 Мои задачи"}, {"text": "🛒 Покупки"}],
        [{"text": "💡 Мои идеи"}, {"text": "📊 Отчёт"}],
        [{"text": "🧹 Очистить"}, {"text": "❓ Помощь"}],
        [{"text": "🔑 Код для чата"}],
    ], "resize_keyboard": True}

BUTTON_CMDS = {
    "🏠 Мой чат": "/mychat", "💬 Выбрать чат": "/pickchat",
    "🎙 Напомнить": "/mode_remind", "📝 Запомнить": "/mode_note",
    "📋 Мои задачи": "/tasks", "🛒 Покупки": "/shop",
    "💡 Мои идеи": "/ideas", "📊 Отчёт": "/report",
    "🔑 Код для чата": "/chatcode", "🧹 Очистить": "/cleanup", "❓ Помощь": "/help",
}

def kb_clmenu():
    return kb([
        [("✅ Выполненные", "clq:done"), ("🛒 Купленное", "clq:shop")],
        [("🎙 Все напоминания", "clq:timed"), ("📝 Все заметки", "clq:ideas")],
        [("⚠️ Удалить ВСЁ", "clq:all"), ("✖ Отмена", "clq:no")],
    ])

MODE_ICON = {"auto": "🤖", "remind": "🎙", "note": "📝"}
MODE_NAME = {"auto": "Авто", "remind": "Напомнить", "note": "Запомнить"}
...
CMD_SYNONYMS = {
    ...
    "/mychat": ["мой чат", "к моему чату", "мой личный чат"],
    "/pickchat": ["выбрать чат", "перейти в чат", "покажи чаты"],
    "/chats": ["мои чаты", "список чатов", "какие чаты", "в каких чатах"],
    "/chatcode": ["код для чата", "дай код", "код чата", "получить код", "код для присоединения"],
    ...
    "/clear_ideas": ["очистить идеи", "удалить заметки", "удалить идеи", "очистить заметки", "очистить заметки"],
    "/clear_timed": ["очистить напоминания", "удалить напоминания", "очистить список напоминаний", "убрать все напоминания"],
...
def parse_task_ex(raw):
    """День не указан — сегодня, время не указано — через 30 минут.
    Возвращает (текст, время, подпись_дня, время_было_явно_названо)."""
    ...
    h = m = None
    hm = re.search(r"(?:в|на|к)\s+(\d{1,2})[:.](\d{2})", text)
    honly = re.search(r"\b(?:в|на|к)\s+(\d{1,2})\s*(часа?|часов)?\b", text)
    numw = next((w for w in NUMW if re.search(rf"\b(?:в|на|к)\s+{w}\b", text)), None)
    ...
    if not text:
        text = "Напоминание"
    return text, target, day_label, explicit

def parse_task(raw):
    return parse_task_ex(raw)[:3]
...
ACTIONS = ["взять", "выбросить", "выброси", "позвонить", "сделать", "отнести",
           "подписать", "оплатить", "написать", "собрать", "приготовить", "сходить",
           "забрать", "отправить", "вынести", "погладить", "сдать", "полить"]

def add_tasks(d, owner_uid, text, source, mode="auto"):
    """Добавляет задачи. Покупки с запятыми -> отдельные пункты.
    Длинная диктовка с несколькими действиями -> отдельные задачи на общее время.
    mode: 'auto' (по ключевым словам), 'note' (всё в заметки), 'remind' (всё напоминания)."""
    icon, cat = detect_cat(text)
    if mode == "note":
        icon, cat = "💡", "идея"
    elif mode == "remind" and cat == "идея":
        icon, cat = "🗒️", "дело"
    u = d["users"].setdefault(owner_uid, new_profile("Пользователь"))
    created = []
    # диктовка: несколько действий через запятые
    acts = [a for a in ACTIONS if a in text]
    if cat != "покупка" and text.count(",") >= 1 and (len(acts) >= 2 or mode == "note"):
        _ft, full_due, full_dl, explicit = parse_task_ex(text)
        parts = [pp.strip() for pp in text.split(",") if pp.strip()]
        if len(parts) >= 2:
            for pp in parts[:8]:
                ttext, due, dl, ex = parse_task_ex(pp)
                if not ex and explicit:
                    due, dl = full_due, full_dl
                ic, ct = detect_cat(pp)
                if mode == "note":
                    ic, ct, due, dl = "💡", "идея", None, "заметка"
                elif mode == "remind" and ct == "идея":
                    ic, ct = "🗒️", "дело" 
                tid = make_task(u, ttext, due, dl, ic, ct, source)
                created.append((f"{ic} {ttext}", due, dl, tid))
            return created
    if cat == "покупка":
        ...
        if len(items) >= 2:
            for it in items[:10]:
                ...
                tid = make_task(u, f"{it}", due, dl, "🛒", "покупка", source)
                created.append((f"🛒 {it}", due, dl, tid))
            return created
    ttext, due, dl = parse_task(text)
    if cat == "идея":
        due, dl = None, "заметка"
    if mode == "remind" and cat == "идея":
        due, dl = parse_task(text)[1], parse_task(text)[2]
        ttext, due, dl = parse_task(text)
...
def display_tasks(u):
    """Единый порядок показа: напоминания по времени, затем заметки."""
...
def render_lists(u, title="📋 Задачи"):
    ...
    for i, t in enumerate(timed[:20], 1):
        lines.append(f"{i}. {'✅' if t['done'] else '•'} {t['day_label']} в {t['time']} — {t.get('icon','🗒️')} {t['text']}")
    if notes:
        lines.append("")
        lines.append(f"💡 Идеи, номера и заметки ({len(timed) + 1}–{len(timed) + len(notes)}):")
        for j, t in enumerate(notes[:20], len(timed) + 1):
            lines.append(f"{j}. {'✅' if t['done'] else '•'} {t.get('icon','💡')} {t['text']}")
    lines.append("")
    lines.append("Изменить: «измени 2 новый текст» · Удалить: «удали 3»")
    return "\n".join(lines)
...
def do_clear(d, owner_uid, mode):
    ...
    if mode == "done":
        ...
        u["tasks"] = [t for t in u["tasks"] if not t.get("due")]
    save_data(d)
...
def handle_command(d, uid, chat, text, username=None):
    ...
    if text == "/help":
        send(chat, "❓ Что я умею\n\n"
                   ...
                   "• Понимаю: «завтра», «в среду», «в 6 вечера», «через час»\n"
                   ...
                   "📒 ЗАДАЧНИКИ\n"
                   "• «задачник дяди» / «мой задачник» — переключение\n"
                   "• /code → семья пишет /link 1234 → /share_on — общий задачник\n\n"
                   "🧹 ОЧИСТКА\n"
                   "• Кнопка «🧹 Очистить» или: «очистить напоминания», «очистить заметки», «удалить всё»\n"
                   "• «измени 2 новый текст» / «удали 3» — по номерам из списка")
        return True
    if not u:
    ...
    if text == "/report":
        ...
        return True
    if text in ("/mode_note", "/mode_remind"):
        want = "note" if text == "/mode_note" else "remind"
        cur = u.get("mode", "auto")
        u["mode"] = "auto" if cur == want else want
        save_data(d)
        m = u["mode"]
        if m == "auto":
            send(chat, "🤖 Режим «Авто»: сам понимаю — «запомни/номер» в заметки, остальное в напоминания.", rkb=main_kb())
        elif m == "note":
            send(chat, "📝 Режим «Запомнить» ВКЛ.\nВсё, что вы скажете или напишете, уйдёт в заметки без напоминаний. Ещё раз нажмите — вернусь в авто.", rkb=main_kb())
        else:
            send(chat, "🎙 Режим «Напомнить» ВКЛ.\nВсё, что вы скажете или напишете, станет напоминанием. Ещё раз нажмите — вернусь в авто.", rkb=main_kb())
        return True
    if text == "/mychat":
        d["users"][uid]["active"] = "self"
        save_data(d)
        send(chat, "🏠 Теперь вы в своём личном чате", rkb=main_kb())
        return True
    if text in ("/chats", "/notebooks"):
        acc = accessible(d, uid)
        cur = active_owner(d, uid)
        lines = ["💬 Ваши чаты:"]
        for i, (ou, nm) in enumerate(acc, 1):
            lines.append(f"{'▶️' if ou == cur else '•'} {i}. {nm}{' (общий)' if ou != uid else ' (мой)'}")
        lines.append("")
        lines.append("Переключиться: кнопка «💬 Выбрать чат» или «выбрать чат» голосом.")
        lines.append("Новый чат: «🔑 Код для чата» — дайте код, человек пишет /join 1234.")
        send(chat, "\n".join(lines), rkb=main_kb())
        return True
    if text == "/pickchat":
        acc = accessible(d, uid)
        ...
        rows = []
        for i, (ou, nm) in enumerate(acc, 1):
            mark = "▶️ " if ou == cur else ""
            rows.append([(f"{mark}{i}. {nm}", f"sw:{i}")])
        rows.append([("🔑 У меня есть код", "sw:join")])
        send(chat, "💬 Выберите чат:", kb=kb(rows))
        return True
    if text == "/chatcode":
        pass  # обработается ниже как /code
    if text == "/code":
        import random
        ...
        return True
    if text.startswith("/link") or text.startswith("/join"):
        parts = text.split()
        if len(parts) < 2:
    ...
    if text.startswith("/repeat"):
        ...
        return True
    if text == "/cleanup":
        send(chat, "🧹 Что очищаем?", kb=kb_clmenu())
        return True
    if text in ("/clear_done", "/clear_all", "/clear_shop", "/clear_ideas", "/clear_timed"):
...
def process_text(d, uid, chat, text, username=None):
    ...
    if text.startswith("/"):
        if handle_command(d, uid, chat, text.split("@")[0], username):
            return
    # изменить/удалить по номеру: «измени 2 купить молоко завтра», «удали 3»
    mnum = re.match(r"(измени|изменить|удали|удалить)\s+(\d+)\s*(.*)", low)
    if mnum:
        act, num = mnum.group(1), int(mnum.group(2))
        rest = text.split(None, 2)
        rest_text = rest[2] if len(rest) > 2 else ""
        owner2 = d["users"][active_owner(d, uid)]
        tasks = display_tasks(owner2)
        if not (1 <= num <= len(tasks)):
            send(chat, f"Нет задачи №{num}. Посмотрите: «мои задачи»", rkb=main_kb())
            return
        t = tasks[num - 1]
        if act.startswith("удал"):
            owner2["tasks"] = [x for x in owner2["tasks"] if x["id"] != t["id"]]
            save_data(d)
            send(chat, f"🗑 Удалена задача {num}: «{t['text']}»", rkb=main_kb())
            return
        if not rest_text:
            send(chat, "Формат: измени 2 купить молоко завтра в 11", rkb=main_kb())
            return
        ttext, due, dl, explicit = parse_task_ex(rest_text)
        ic, ct = detect_cat(rest_text)
        if ct == "идея":
            due, dl = None, "заметка"
        t.update({"text": ttext, "due": due.isoformat() if due else None,
                  "time": due.strftime("%H:%M") if due else "",
                  "day_label": dl, "cat": ct, "icon": ic})
        save_data(d)
        when = f"{dl} в {due.strftime('%H:%M')}" if due else dl
        send(chat, f"✏️ Задача {num} обновлена: {ic} {ttext} ({when})", rkb=main_kb())
        return
    # новая задача (или список покупок) — с учётом режима 🎙/📝
    u_self = d["users"].setdefault(uid, new_profile("Я"))
    created = add_tasks(d, active_owner(d, uid), text, "self", mode=u_self.get("mode", "auto"))
    save_data(d)
    m = u_self.get("mode", "auto")
    mpref = {"note": "📝 ", "remind": "🎙 "}.get(m, "")
    if len(created) > 1:
        lines = [f"{mpref}Добавил {len(created)} позиций:"] + [c[0] for c in created]
        send(chat, "\n".join(lines), rkb=main_kb())
    else:
        label, due, dl, tid = created[0]
        when = f"{dl} в {due.strftime('%H:%M')}" if due else dl
        send(chat, f"{mpref}📌 {label}\n{when}", kb=kb_task(tid))
        if SEND_VOICE:
            try:
...
def handle_callback(d, cb):
    ...
    if parts[0] == "done":
        ...
        if not t:
            return
        mins = int(parts[2])
        t["status"] = "snoozed"; t.pop("quiet", None)
        ...
        if i == "join":
            tg("editMessageText", chat_id=cb["message"]["chat"]["id"],
               message_id=cb["message"]["message_id"],
               text="🔑 Напишите код: /join 1234 (код выдаёт владелец чата кнопкой «🔑 Код для чата»)")
            return
        acc = accessible(d, uid)
        try:
            ou, nm = acc[int(i) - 1]
            ...
            tg("editMessageText", chat_id=cb["message"]["chat"]["id"],
               message_id=cb["message"]["message_id"], text="Чат не найден. Попробуйте снова.")
            return
        d["users"].setdefault(uid, new_profile("Я"))["active"] = ou if ou != uid else "self"
        save_data(d)
        tg("editMessageText", chat_id=cb["message"]["chat"]["id"],
           message_id=cb["message"]["message_id"],
           text=f"💬 Вы в чате «{nm}»")
        ...
        if mode == "no":
            tg("editMessageText", chat_id=cb["message"]["chat"]["id"],
               message_id=cb["message"]["message_id"], text="✖ Отменено")
            return
        kb2 = json.dumps(kb_confirm(mode), ensure_ascii=False)
        tg("editMessageReplyMarkup", chat_id=cb["message"]["chat"]["id"],
           message_id=cb["message"]["message_id"], reply_markup=kb2)
        tg("editMessageText", chat_id=cb["message"]["chat"]["id"],
           message_id=cb["message"]["message_id"],
           text=f"🧹 Точно удалить {CLEAR_TEXT[mode]}?")
    elif parts[0] == "clr":
            ...
            if now >= due and (last is None or (now - last).total_seconds() >= repeat_minutes(u) * 60):
                ...
                if SEND_VOICE:
                    try:
                        ...
                        log.warning(f"TTS: {e}")
                if t["fires"] == 8:
                    send(int(k), f"🌙 «{t['text']}» — замолкаю до лучших времён, ночь на дворе. Подтвердите кнопкой утром.")
                    t["status"] = "quiet"; changed = True
                    continue
